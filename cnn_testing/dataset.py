...
def load_data(dataset_path, logger=None):
    # Automatic search outside current root (check parent directory's datasets)
    if not os.path.exists(dataset_path):
        parent_ds_path = os.path.join("..", dataset_path)
        if os.path.exists(parent_ds_path):
            dataset_path = parent_ds_path
        else:
            # Also check specifically in ../2d_datasets/ours/
